...
public class SertifikatiCRUDService implements ISertifikatiService {
    ...
    public Sertifikati retrieveSertifikatiById(long sertId) throws Exception {
    	log.info("Mēģina atrast sertifikātu ar ID: {}", sertId);
        if (sertId <= 0) {
    ...
    {
    	log.info("Sāk atjaunināt sertifikātu ar ID: {}", id);
    ...
    public void deleteSertifikatiById(int id) throws Exception {
    	log.info("Sāk dzēst sertifikātu ar ID: {}", id);
        if (id <= 0) {
        ...
        log.debug("Dzēš ierakstus priekš sertifikāta ID: {}", id);
